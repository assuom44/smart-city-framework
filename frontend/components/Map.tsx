...
export default function Map() {
  ...
  const currentStats = isTrafficMode
    ...
    : isWaterMode
      ? waterStats
      : envStats;
  const currentConnectedd = isTrafficMode
    ? trafficConnectedd
    : isWaterMode
      ? waterConnectedd
      : envConnectedd;
      ...
      {isModalOpen && (
        <Modal isOpen={isModalOpen} onClose={closeModal}>
          <Controls
            ...
            kafkaConnectedd={currentConnectedd}
      ...
      <div className="flex-1 relative h-full min-w-0">
        ...
        {!currentLoading && currentError && (
          <div className="absolute top-28 left-6 right-6 z-1001 pointer-events-none">
            <StateNotice
              variant="error"
              message={`${currentError}. The displayed data may be stale.`}
              className="mx-auto max-w-2xl pointer-events-auto"
            />
          </div>
        )}

        {!currentLoading && !currentError && !currentConnectedd && !currentHasData && (
          <div className="absolute top-28 left-6 right-6 z-1001 pointer-events-none">
            <StateNotice
              variant="disconnected"
              message="The backend is not connected. Start the API and Kafka producers to populate the map."
              className="mx-auto max-w-2xl pointer-events-auto"
            />
          </div>
        )}
