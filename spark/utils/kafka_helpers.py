...
def create_kafka_stream(spark: SparkSession, topic_name: str, kafka_servers: str):
    """
    Create a Spark Structured Streaming source from a Kafka topic.
    """
    return spark \
        .readStream \
        .format("kafka") \
...
def create_raw_kafka_lake_stream(spark: SparkSession, topic_name: str, kafka_servers: str, source: str):
    """
    Prepare a raw Kafka stream for the bronze data-lake layer.
    """
...
def parse_kafka_json_records(df, element_schema, stream_name: str):
    """
    Parse Kafka messages as JSON objects or arrays and separate invalid messages.
    """
...
def write_stream_to_console(df, query_name: str):
    """
    Write an aggregated DataFrame to the console.
    """
    return df \
        .writeStream \
        .outputMode("update") \
...
def write_stream_to_kafka(
    ...
    """
    Convert the DataFrame to JSON and publish it to a Kafka topic.
    """
    from pyspark.sql.functions import to_json, struct

    # Kafka expects a "value" column containing the message as a JSON string.
