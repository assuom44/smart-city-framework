...
async function startKafkaConsumer(io) {
  ...
  console.log("Kafka Consumer Connectedd");
  ...
  await consumer.run({
    eachMessage: async ({ topic, message }) => {
      try {
        ...
        // Raw readings are emitted for live updates; they are not persisted by this consumer.
        ...
        // Spark topics are persisted for historical views and emitted for real-time updates.
...
async function stopKafkaConsumer() {
  try {
    ...
    console.warn("[Kafka] Consumer disconnect warning:", error.message);
  }
}
